...
enum CyberTheme {
    ...
    static let displayFont = Font.system(.title, design: .serif).weight(.heavy)
...
extension View {
    func cyberGlow(_ color: Color = CyberTheme.cyberPurple, radius: CGFloat = 16, opacity: Double = 0.7) -> some View {
...
struct CyberCard<Content: View>: View {
    ...
    var body: some View {
        content()
            ...
            .shadow(color: CyberTheme.cyberPurple.opacity(0.15), radius: 12, x: 0, y: 0)
    }
}
...
// MARK: - iOS 15 Safe Scroll Indicators
