...
final class AppViewModel: ObservableObject {
    ...
    func restoreSessionIfNeeded() async {
        ...
        let savedKey: String? = KeychainManager.read(key: "saved_license_key")
            ?? UserDefaults.standard.string(forKey: "nexora_license_key")
    ...
    func verifyKey(silent: Bool = false) async {
        ...
        guard let encodedKey = trimmed.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed),
              ...
              let url = URL(string: "https://gialuong.onrender.com/api/verify?key=\(encodedKey)&hwid=\(encodedHWID)&secret=\(encodedSecret)") else {
    ...
    func logout() {
        ...
        stopMonitor()
        clearSession()
        showToast("Đã đăng xuất hệ thống Nexora.", type: .warning)
    }

    private func clearSession() {
        clearSavedCredentials()
        UserDefaults.standard.removeObject(forKey: "nexora_license_key")
        keyInput = ""
        authResult = nil
    ...
    func checkServerConnection() async -> Bool {
        guard let url = URL(string: "https://gialuong.onrender.com/api/health") else { return false }
    ...
    private func reverifySilent() async {
        ...
        guard let encodedKey = savedKey.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed),
              let encodedHWID = hwid.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed),
              let encodedSecret = secret.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed),
              let url = URL(string: "https://gialuong.onrender.com/api/verify?key=\(encodedKey)&hwid=\(encodedHWID)&secret=\(encodedSecret)") else { return }
        do {
