...
struct LoginView: View {
    ...
    var body: some View {
        ...
        .alert("Mã thử nghiệm", isPresented: $showTrialAlert) {
            ...
            Text("Đã điền mã NEXORA-VIP-TEST. Nhấn XÁC THỰC NGAY để tiếp tục.")
        }
    }
    ...
    private var notificationBanner: some View {
        if !viewModel.serverConnected {
            HStack(spacing: 8) {
                ...
                Text("Máy chủ Nexora đang ngoại tuyến. Một số tính năng có thể không hoạt động.")
    ...
    private var activationCard: some View {
        CyberCard {
            VStack(spacing: 12) {
                Text("KÍCH HOẠT HỆ THỐNG NEXORA")
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text("LICENSE KEY")
                        ...
                        .foregroundStyle(CyberTheme.textSecondary)
                        .tracking(1.5)
                    TextField("NEXORA-XXXX-XXXX", text: $viewModel.keyInput)
                ...
                Button {
                    Haptics.light()
                    viewModel.keyInput = "NEXORA-VIP-TEST"
                    showTrialAlert = true
                } label: {
    ...
    private var contactRow: some View {
        HStack(spacing: 10) {
            ...
            contactPill(icon: "bubble.left.fill", label: "Zalo", tint: CyberTheme.cyberGold) {
                copyAndOpen(text: "0862164381", toast: "Đã sao chép SĐT Zalo Gia Lượng!", openURL: "tel:0862164381")
            }
        }
    }
