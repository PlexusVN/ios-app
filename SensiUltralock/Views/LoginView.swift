...
struct LoginView: View {
    ...
    private var gridBackground: some View {
        Canvas { context, size in
            let spacing: CGFloat = max(28, w * 0.12)
            for x in stride(from: 0, through: size.width, by: spacing) {
                var path = Path()
                path.move(to: CGPoint(x: x, y: 0))
                path.addLine(to: CGPoint(x: x, y: size.height))
                context.stroke(path, with: .color(CyberYellow.opacity(0.06)), lineWidth: 1)
            }
    ...
    private var brandingSection: some View {
        VStack(spacing: h * 0.004) {
            Text("Sensi Ultralock")
                .font(.custom("Georgia", size: max(22, 30 * scale)).italic().weight(.black))
    ...
    private var authCard: some View {
        VStack(spacing: h * 0.018) {
            ...
            Button(action: loginAction) {
                ...
                .cornerRadius(max(12, 14 * scale))
                .shadow(color: CyberYellow.opacity(0.3), radius: w * 0.02)
            }
