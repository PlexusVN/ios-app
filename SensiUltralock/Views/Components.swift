...
struct FeatureCardView: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 11)
        .background(isLocked ? CyberDarkBg.opacity(0.5) : CyberDarkBg)
        .cornerRadius(14)
        .overlay(RoundedRectangle(cornerRadius: 14).stroke(borderColor, lineWidth: 1))
        .onAppear { updateBorder() }
        .onChange(of: isChecked) { _ in updateBorder() }
            ...
            HStack(spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    Text(platform.uppercased())
                        .font(.system(size: 8, weight: .bold)).foregroundColor(CyberTextSecondary).kerning(0.4)
                    Text(value)
